...
def test_pluralize(count: int, word: str, expected: str) -> None:
...
def test_is_dir_writable_creates_dir_if_missing(
    tmp_path: Path, create_flag: bool
) -> None:
...
def test_is_dir_writable_returns_false_if_unwritable_posix(
...
def test_is_dir_writable_simulates_unwritable(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    called = {"yes": False}
